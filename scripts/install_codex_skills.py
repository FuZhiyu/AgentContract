...
def read_plugin_name(plugin_dir: Path) -> str | None:
    for manifest_relpath in (
        ...
        plugin_json = plugin_dir / manifest_relpath
...
def write_agent_role_file(
    ...
    content = strip_frontmatter(source.agent_md.read_text(encoding="utf-8")).strip()
    header_lines = [f"Role: {source.agent_name}"]
    if source.description:
        header_lines.append(f"Description: {source.description}")
    header = "\n".join(header_lines)
    developer_instructions = f"{header}\n\n{content}\n"
    payload = [
